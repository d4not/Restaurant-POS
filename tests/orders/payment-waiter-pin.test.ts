...
// During a PROVISIONAL shift the floor staff opener is implicitly the
// approver for payments — they don't have to walk back and find a cashier
// PIN, since by definition the provisional was opened because the regular
// cashier is busy. The waiter is recorded on Payment.approved_by_user_id.
describe('Payment — waiter/barista on a PROVISIONAL shift bypasses the cashier-PIN requirement', () => {
  it('waiter on provisional shift pays without PIN; approved_by reflects the waiter', async () => {
    // Custom seed: admin opens a parent REGULAR shift (required by the
    // report-spec provisional contract), then a waiter opens a provisional
    // attached to it. Orders/payment go through the provisional.
    const [admin, waiter, supplier, barra, cat] = await Promise.all([
      makeUser({ role: 'ADMIN' }),
      makeUser({ role: 'WAITER', pin: '5921' }),
      makeSupplier({ name: 'Distribuidora Test' }),
      makeStorage({ name: 'Barra' }),
      makeSupplyCategory({ name: 'Bottled Drinks' }),
    ]);
    ...
    // Parent REGULAR shift — required for the provisional to attach to.
    ...
    // Provisional shift opened by the waiter — cashier is on the parent
    // shift but tied up elsewhere, so the waiter runs this side-flow.
    const reg = await request(app)
      .post('/api/v1/registers/provisional')
      .set(waiterAuth)
      .send({ parent_shift_id: parent.body.data.id })
      .expect(201);
    expect(reg.body.data.type).toBe('PROVISIONAL');
    expect(reg.body.data.kind).toBe('PROVISIONAL');

    await request(app)
      .post('/api/v1/deduction-rules')
      .set(adminAuth)
      .send({ pos_register_id: reg.body.data.id, storage_id: barra.id })
      .expect(201);
    ...
    // No PIN provided — would 403 on a NORMAL shift, but provisional waives.
    const pay = await request(app)
    ...
    expect(stored.approved_by_user_id).toBe(waiter.id);
  });
});
